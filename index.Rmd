...
# readRDS("avg_weekly_calls_by_yeartype2.rds")
...
predictor_str_units <- shiny::reactive({
  ...
  if(is.null(model_data())) {
    ...
    return(NULL)
       ...
       pred_str <- model_data()$predictor_long_name[1]
       
       if (pred_str == "May 1 SWE") {
         return("May 1 SWE (inches)")
         
       } else if(pred_str == "NRCS May 1 Apr-Sep forecast") {
         
         return("NRCS May 1 Apr-Sep forecast (kAF)")
         
       } else {
         
         return("Peak SWE (inches)")
...
# output$predictor_label <- shiny::renderUI({
#   if (is.null(predictor_long_str())) {
#     return(HTML("Enter a predictor value after selecting a district..."))
#   } else {
...
# shiny::uiOutput("predictor_label")
# # string containing the predictor for the given district
...
###
```{r}
# paste0("<strong>Predictor Variables:</strong> these are the variables included in this districts. <strong>MLR model.Prediction:</strong> This is the average summer call year that results from the two predictor variable values above. This means that water rights with appropriation dates after this predicted year are likely to be out of priority and receive no water. <strong>Model equation:</strong> This is the best fitting MLR model given the available predictor variables. <strong>Performance:</strong> The higher the R² value, the better the performance of the model.")
# # output prediction value box UI
gt::gt_output("gtModelTable")
shinyBS::bsPopover(
  "gtModelTable",
  title     = "Interpretation of MLR",
  # content   = "Predictor Variables: these are the variables included in this districts MLR model.Prediction: This is the average summer call year that results from the two predictor variable values above. This means that water rights with appropriation dates after this predicted year are likely to be out of priority and receive no water. Model equation: This is the best fitting MLR model given the available predictor variables. Performance: The higher the R² value, the better the performance of the model.",
...
<!--   flexdashboard::valueBox( -->
<!--     value = paste0(primary_pred_valuebox_label()), -->
...
<!--   # flexdashboard::valueBox( -->
<!--   #   # value = paste0("Predictor: ", round(as.numeric(input$newDataInput), 2)), -->
<!--   #   # value = paste0(predictor_long_str(), " ", round(as.numeric(input$newDataInput), 2)), -->
<!--   #   value = paste0(predictor_long_str(), " ", primary_pred_valuebox_label()), -->
<!--   #   color = "danger", -->
<!--   #   caption = "" -->
<!--   #   ) -->
<!-- ) -->
...
<!-- ```{r context="server"} -->
<!-- # x = 5 -->
<!-- #   ifelse(is.na(x), " 454", 5) -->

<!-- # label_val = ifelse( -->
<!-- #     is.na(round(as.numeric(input$newDataInput), 2)), -->
<!-- #     " ", -->
<!-- #     round(as.numeric(input$newDataInput), 2) -->
<!-- #     ) -->
<!-- # # Render the prediction value Value Box -->
...
<!--     # value = paste0(predictor_long_str2(), " ", round(eddi_val(), 2)), -->
<!--     # value = paste0(round(eddi_val(), 2)), -->
<!--     value = paste0(eddi_pred_valuebox_label()), -->
<!--     color = "danger", -->
...
<!--   # flexdashboard::valueBox( -->
<!--   #   value = paste0(predictor_long_str2(), " ", round(eddi_val(), 2)), -->
<!--   #   # value = paste0(predictor_long_str2(), " ", eddi_valuebox_label()), -->
<!--   #   color = "danger", -->
<!--   #   caption = "" -->
<!--   #   ) -->
<!-- ) -->
